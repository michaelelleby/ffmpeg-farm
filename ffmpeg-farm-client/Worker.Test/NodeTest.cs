...
namespace Worker.Test
{
    ...
    public class NodeTest
    {
        ...
        public async Task CanWaveAndStereoToolPreset()
        {
            ...
            {
                Arguments = $"{{FFMpegPath}} -i \"{srcFilePath}\" -f wav -hide_banner -loglevel info - | \"{{StereoToolPath}}\" - {(writeOutputFileToDisk ? $"\"{stereoToolOutputPath}\"" : "NUL")} -s \"{{StereoToolPresetsPath}}{Path.DirectorySeparatorChar}Ossian_Px_7threads_ny.sts\"{{StereoToolLicense}} -q",
                FfmpegJobsId = 1,
                Id = 10,
